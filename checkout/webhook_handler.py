...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        if order_exists:
            ...
            message = '| SUCCESS: Verified order already in database'
            return HttpResponse(
                content=f'Webhook received: {event["type"]} {message}',
                status=200,
                )
        else:
            order = None
            try:
                ...
                for item_id, item_data in json.loads(bag).items():
                    ...
                    if isinstance(item_data, int):
                        ...
                        for size, q in item_data["items_by_size"].items():
                            order_line_item = OrderLineItem(
                                order=order,
                                product=product,
                                quantity=q,
        ...
        self._send_confirmation_email(order)
        message = '| SUCCESS: Created order in webhook'
        return HttpResponse(
            content=f'Webhook received: {event["type"]} {message}',
            status=200,
        )
